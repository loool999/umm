...
def adjust_object_image_color(obj_img, base_img):
    obj_img_copy = obj_img.copy()
    obj_img_copy.thumbnail(base_img.size, Image.LANCZOS)
    base_img_copy = base_img.copy()
    base_img_copy.paste(obj_img_copy, (0, 0), obj_img_copy)
    ...
    enhancer = ImageEnhance.Color(obj_img)
    obj_img_adjusted = enhancer.enhance(2.0)
...
def paste_object_image(base_image_path, object_folder, output_v1_image_path):
    with Image.open(base_image_path) as base_img:
        ...
        with Image.open(object_image_path) as obj_img:
            ...
            obj_img = adjust_object_image_color(obj_img, base_img)
    ...
    for phase in range(num_phases):
        ...
        for _ in range(iterations):  # Adjust the number of iterations as needed
            ...
            if score > last_similarity_score:
                ...
                best_output_path = os.path.join(process_folder, f'{_}_best_output_phase_{phase + 1}.png')
                os.rename(output_v1_image_path, best_output_path)
                print(f"{_} New best image saved with similarity score: {score:.2f}%")
            else:
                os.remove(output_v1_image_path)
                print(f"{_} Discarded image with similarity score: {score:.2f}%")
        if best_output_path and os.path.exists(best_output_path):
            phase_best_output_path = os.path.join(phase_folder, f'{_}_best_output_phase_{phase + 1}.png')
            os.rename(best_output_path, phase_best_output_path)
            base_image_path = phase_best_output_path
        else:
            print(f"{_} No new best image found in phase {phase + 1}")

process_images(input_image_path, output_image_path, object_folder, process_folder, phase_folder, num_phases=5, iterations=100)
